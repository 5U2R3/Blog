...
VirusTotal上では、当該IPに紐づくPassive DNS項目の中で `fidufagios[.]com` が最も多く検出されていた。
...
PCAPに記録されたDNSクエリとVirusTotalのPassive DNS情報を照合することで、当該IPに関連するドメイン候補を絞り込める。
...
Q4で特定したドメイン `fidufagios.com` をVirusTotalで検索し、`Relations` タブの `Communicating Files` を参照する。
